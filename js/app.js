...
(function () {
  ...
  const S = window.PPState = {
    city:   q.get('city')   || 'All Regions',
    sm:     q.get('sm')     || 'All SM',
    status: q.get('status') || 'All Status',
    projectStatus: q.get('projectStatus') || 'All Project Status',
    source: q.get('source') || 'All Sources',
    developer: q.get('developer') || 'All Developers',
  ...
  const qs = () => {
    ...
    ['city','sm','status','projectStatus','source','developer','from','to'].forEach(k => p.set(k, S[k]));
    return p.toString();
  };
  window.PPqs = qs;
  ...
  const PROJECT_STATUSES = ['All Project Status','Builtup','Sustenance','Realised'];
  /* Real paid-media sources, derived from the MIS Source Wise data (All Regions),
     ranked by full-period spend — replaces the old placeholder list so the filter
     actually matches what's in sourceWise / campaigns. */
  const SOURCES = ['All Sources'].concat(
    (window.PP.sourceWise['All Regions'] || [])
      .filter(r => r.cost > 0)
      .sort((a, b) => b.cost - a.cost)
      .map(r => r.src)
  );
  const DEVELOPERS = ['All Developers','Lodha','Godrej','Prestige','Sobha'];

  function smsFor(city) {
    if (city === 'Bangalore') return BLR_SMS.slice();
    if (city === 'All Regions') {
      return ['All SM'].concat([...new Set(window.PP.smWise.map(r => r.sm))]);
    }
    const list = window.PP.smWise.filter(r => r.city === city).map(r => r.sm);
    return ['All SM'].concat([...new Set(list)]);
  }
  ...
  window.projectStatusMatch = (rowValue, filter) => {
    if (filter === 'All Project Status') return true;
    return String(rowValue || '').trim().toLowerCase() === filter.toLowerCase();
  };
  window.sourceMatch = (rowValue, filter) => {
    if (filter === 'All Sources') return true;
    return String(rowValue || '').trim().toLowerCase() === filter.toLowerCase();
  };
  window.developerMatch = (rowValue, filter) => {
    if (filter === 'All Developers') return true;
    return String(rowValue || '').trim().toLowerCase() === filter.toLowerCase();
  };
  ...
  function renderFilters() {
    ...
    el.innerHTML = `
      <div class="f-group"><label for="fCity">Select City</label>
        <select id="fCity">${CITIES.map(c => `<option ${c === S.city ? 'selected' : ''}>${c}</option>`).join('')}</select></div>
      <div class="f-group"><label for="fSM">SM Name</label><select id="fSM"></select></div>
      <div class="f-group"><label for="fStatus">Campaign Status</label>
        <select id="fStatus">${['All Status','Live','Pause','Hold'].map(s => `<option ${s === S.status ? 'selected' : ''}>${s}</option>`).join('')}</select></div>
      <div class="f-group"><label for="fProjectStatus">Project Status</label>
        <select id="fProjectStatus">${PROJECT_STATUSES.map(s => `<option ${s === S.projectStatus ? 'selected' : ''}>${s}</option>`).join('')}</select></div>
      <div class="f-group"><label for="fSource">Source</label>
        <select id="fSource">${SOURCES.map(s => `<option ${s === S.source ? 'selected' : ''}>${s}</option>`).join('')}</select></div>
      <div class="f-group"><label for="fDeveloper">Developer</label>
        <select id="fDeveloper">${DEVELOPERS.map(s => `<option ${s === S.developer ? 'selected' : ''}>${s}</option>`).join('')}</select></div>
      <div class="f-group"><label>Date Range</label>
        <div class="daterange">
          <input type="date" id="fFrom" value="${S.from}"><span>\u2013</span>
          <input type="date" id="fTo" value="${S.to}">
        </div></div>`;
    fillSM();
    el.querySelector('#fCity').onchange = e => {
      S.city = e.target.value;
      const opts = smsFor(S.city);
      S.sm = opts.includes(S.sm) ? S.sm : 'All SM';
      fillSM(); emit();
    };
    el.querySelector('#fSM').onchange = e => { S.sm = e.target.value; emit(); };
    el.querySelector('#fStatus').onchange = e => { S.status = e.target.value; emit(); };
    el.querySelector('#fProjectStatus').onchange = e => { S.projectStatus = e.target.value; emit(); };
    el.querySelector('#fSource').onchange = e => { S.source = e.target.value; emit(); };
    el.querySelector('#fDeveloper').onchange = e => { S.developer = e.target.value; emit(); };
    el.querySelector('#fFrom').onchange = e => { S.from = e.target.value; emit(); toast('Period updated \u2014 figures reflect the 5 Aug 2026 MIS snapshot'); };
    el.querySelector('#fTo').onchange = e => { S.to = e.target.value; emit(); toast('Period updated \u2014 figures reflect the 5 Aug 2026 MIS snapshot'); };

    function fillSM() {
      const sel = el.querySelector('#fSM');
      sel.innerHTML = smsFor(S.city).map(s => `<option ${s === S.sm ? 'selected' : ''}>${s}</option>`).join('');
    }
  }
