...
(function () {
  ...
  const cityKey = () => { const c = window.PPcityLabel(); return (c === 'All Regions' || /regions$/.test(c)) ? 'All Regions' : c; };
  ...
  function renderSource() {
    ...
    document.getElementById('srcNote').innerHTML =
      ...
      (cityKey() === 'All Regions' ? 'all-region' : cityKey()) + ' figures on the ROI Summary. QL and SV per source come from the Spend Tracker <b>QL LEADS DATA</b> sheet. ' +
  ...
  function renderTable() {
    ...
    const ck = cityKey();
    const isBlr = ck === 'Bangalore' || ck === 'All Regions';
    const rows = isBlr ? D.campaigns.filter(c => statusMatch(c.status, S.status)) : [];
    ...
    document.getElementById('campTitle').textContent = (isBlr ? 'Bangalore' : ck) + ' \u2013 Campaign Register';
